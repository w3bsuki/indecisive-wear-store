...
export function BestSellersSection() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const medusaProducts = await medusaService.getProducts(6)
  ...
  const transformedProducts = products.map((product, index) => ({
    id: index + 1, // Use index for frontend ID
    medusaId: product.id,
    name: product.title,
    price: 45 + (index * 15), // Demo pricing since Medusa products don't have prices set
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between mb-4 md:mb-8">
          <div>
            ...
            <p className="text-muted-foreground text-xs md:text-xl font-mono">Most loved by our community</p>
          </div>
          <Button
            variant="outline"
